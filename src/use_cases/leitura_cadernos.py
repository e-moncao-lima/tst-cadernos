...
class LeituraCadernos:
    ...
    def __obter_conteudo_primeira_pagina_arquivo_pdf(self, caminho_arquivo_pdf: str) -> list[str]:
        return manipulacao_pdf.obter_texto_arquivo_pdf(self.logger, caminho_arquivo_pdf)[0]
    ...
    def __encontrar_padrao_numero_processo(self, conteudo_arquivo: str) -> str | None:
        ...
        if padrao_data_disponibilizacao is None or len(padrao_data_disponibilizacao) == 0:
            self.logger.error("Padrão não encontrado.")
            return
    ...
    def __deletar_arquivos_resultados(self) -> None:
        sistema_arquivos.remover_arquivo(self.logger, constantes.CAMINHO_JSON_RESULTADOS)
    
    def __salvar_dados_arquivo(self, numero_processo: str, data_distribuicao: str) -> None:
        dados_atualizados = sistema_arquivos.obter_dados_arquivo_json(self.logger, constantes.CAMINHO_JSON_RESULTADOS)
        ...
        sistema_arquivos.escrever_dados_arquivo_json(self.logger, constantes.CAMINHO_JSON_RESULTADOS, dados_atualizados)

    def executar(self) -> None:
        self.logger.info("Iniciando procedimento de leitura dos cadernos.")
        ...
        self.logger.info("Processo de leitura dos cadernos concluída com sucesso.")

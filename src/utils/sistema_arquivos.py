...
def remover_arquivos_diretorio(logger, caminho_diretorio: str) -> None:
    for arquivo in listar_arquivos_diretorio_por_extensao(caminho_diretorio):
        remover_arquivo(logger, arquivo)


def remover_arquivo(logger, caminho_arquivo: str) -> None:
    try:
        os.remove(caminho_arquivo)
    except FileNotFoundError:
        logger.error("Arquivo não encontrado para deletar.")


def obter_dados_arquivo_json(logger, caminho_arquivo_json: str) -> dict:
    try:
        dados_arquivo = {}
...
def escrever_dados_arquivo_json(logger, caminho_arquivo_json: str, dicionario_dados: dict[str, str]) -> dict:
    try:
